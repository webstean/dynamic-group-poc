resource "msgraph_resource" "company_dynamic_security_groups" {
  ...
  body = {
    ...
    description     = "Dynamic group for enabled member accounts with department == '${each.value}'"
    mailEnabled     = false
...
resource "msgraph_resource" "department_dynamic_security_groups" {
  ...
  body = {
    ...
    description     = "Dynamic group for enabled member accounts with companyName == '${each.value}'"
    mailEnabled     = false
...
resource "msgraph_resource" "exta5_dynamic_security_groups" {
  ...
  body = {
    ...
    description     = "Dynamic group for enabled member accounts with extensionAttribute5 == '${each.value}'"
    mailEnabled     = false
...
resource "msgraph_resource" "exta6_dynamic_security_groups" {
  ...
  body = {
    displayName     = "Dyn-Group-${each.value}-Members"
    description     = "Dynamic group for enabled member accounts with extensionAttribute6 == '${each.value}'"
    mailEnabled     = false
...
resource "msgraph_resource" "exta7_dynamic_security_groups" {
  ...
  body = {
    displayName     = "Dyn-Group-${each.value}-Members"
    description     = "Dynamic group for enabled member accounts with extensionAttribute7 == '${each.value}'"
    mailEnabled     = false
